...
# Temporary file storage for callbacks (to avoid long callback_data)
FILE_STORAGE = {}  # {file_hash: file_content}
...
def validate_expiry(month, year):
    """Check if expiry is in the future - accepts both 2-digit and 4-digit years"""
    try:
        current_year_full = datetime.now().year
        current_year_2digit = current_year_full % 100
        ...
        if year_full < current_year_full:
            return False
        if year_full == current_year_full and month_int < current_month:
            return False
        return True
    except:
        return False
...
# Message splitting helper (Telegram limit: 4096 characters)
...
def send_long_message(bot, chat_id, text, parse_mode='Markdown', reply_to=None):
...
# CC Parser with Validation - Supports both 2-digit and 4-digit years, Amex (15 digits), and all card types
def parse_cc(cc_string):
    # Remove all non-digit and non-pipe characters, but preserve structure
    # Handle spaces, dashes, and other separators
    ...
    if len(parts) >= 4:
        ...
        # Remove any remaining non-digits from card number (spaces, dashes, etc.)
        ...
        # Handle year - accept both 2-digit and 4-digit
        if not year.isdigit():
            return None
        year_original = year
        
        if len(year_original) == 4:
            # 4-digit year: convert to 2-digit for storage
            try:
                year_int = int(year_original)
                year_2digit = year_int % 100
                year = f"{year_2digit:02d}"
            except:
                return None
        elif len(year_original) == 2:
            # Already 2-digit
            year = year_original
        else:
            # Invalid year length
        ...
        # Validate card number - Accept 13-19 digits (covers Visa, Mastercard, Amex, Discover, etc.)
        ...
        # Validate year (should be 2 digits now)
        if len(year) != 2:
            return None
        
        # Validate expiry (pass original year for validation)
        if not validate_expiry(mon, year_original if len(year_original) == 4 else year):
            return None
        
        # Validate CVV - Amex uses 4 digits, others use 3-4
        if not cvv.isdigit():
            return None
        cvv_len = len(cvv)
        if cvv_len < 3 or cvv_len > 4:
            return None
        
        # Amex cards (15 digits) typically have 4-digit CVV
        # But we'll accept 3-4 digits for all cards for flexibility
        if card_len == 15 and cvv_len == 4:
            # Amex with 4-digit CVV - perfect
            pass
        elif card_len == 15 and cvv_len == 3:
            # Amex with 3-digit CVV - also accept (some systems use 3)
            pass
        elif cvv_len < 3 or cvv_len > 4:
            return None
...
def process_bulk(message):
    try:
        ...
        # Send summary first (always fits)
        summary = f"📊 **BULK CHECK COMPLETE**\n\n"
        summary += f"**Total:** {len(cc_list)} cards\n"
        summary += f"💎 **High Limit:** {stats['high_balance']}\n"
        summary += f"✅ **Live:** {stats['live']}\n"
        summary += f"❌ **Dead:** {stats['dead']}\n"
        summary += f"⚠️ **Errors:** {stats['error']}\n"
        summary += f"📈 **Success Rate:** {(stats['live']/len(cc_list)*100):.1f}%" if len(cc_list) > 0 else "0%"
        
        bot.reply_to(message, summary, parse_mode='Markdown')
        
        # Send detailed results in chunks (only if not too many cards)
        if len(cc_list) <= 200:  # Only show details for smaller bulks
            ...
            send_long_message(bot, message.chat.id, output, parse_mode='Markdown', reply_to=message)
        else:
            # For large bulks, just show stats
            bot.send_message(message.chat.id, f"ℹ️ Detailed results skipped for {len(cc_list)} cards. Showing summary only.", parse_mode='Markdown')
        
        # Send live cards details (limited to 10)
        if lives:
            lives_text = "**🎯 LIVE CARDS** (showing first 10):\n\n"
            for i, card_data in enumerate(valid_cards[:10], 1):
                lives_text += f"{i}. `{card_data}`\n"
            if len(valid_cards) > 10:
                lives_text += f"\n... and {len(valid_cards) - 10} more live cards"
            send_long_message(bot, message.chat.id, lives_text, parse_mode='Markdown')
        
        # Send copyable valid cards list (split if needed)
        if valid_cards:
            copyable_text = "📋 **COPY VALID CARDS:**\n\n```\n"
            copyable_text += "\n".join(valid_cards)
            copyable_text += "\n```"
            send_long_message(bot, message.chat.id, copyable_text, parse_mode='Markdown')
...
def process_sk_bulk(message):
    try:
        ...
        # Send summary first
        summary = f"📊 **STRIPE BULK COMPLETE**\n\n"
        summary += f"**Total:** {len(cc_list)} cards\n"
        summary += f"✅ **Live:** {stats['live']}\n"
        summary += f"❌ **Dead:** {stats['dead']}\n"
        summary += f"⚠️ **Errors:** {stats['error']}\n"
        ...
        # Send detailed results in chunks (only if not too many)
        if len(cc_list) <= 200:
            output = f"📋 **DETAILED RESULTS:**\n\n"
            for i, result in enumerate(results, 1):
                card_short = cc_list[i-1].split('|')[0][:19] + "..."
            ...
            bot.send_message(message.chat.id, f"ℹ️ Detailed results skipped for {len(cc_list)} cards.", parse_mode='Markdown')
        
        # Send live cards (limited)
        if lives:
            lives_text = "**🎯 LIVE CARDS** (showing first 10):\n\n"
            for i, card_data in enumerate(valid_cards[:10], 1):
                lives_text += f"{i}. `{card_data}`\n"
            if len(valid_cards) > 10:
                lives_text += f"\n... and {len(valid_cards) - 10} more"
            send_long_message(bot, message.chat.id, lives_text, parse_mode='Markdown')
        
        # Send copyable valid cards (split if needed)
        if valid_cards:
            copyable_text = "📋 **COPY VALID CARDS:**\n\n```\n"
            copyable_text += "\n".join(valid_cards)
            copyable_text += "\n```"
...
def handle_document(message):
    ...
    try:
        ...
        # Store file content temporarily with a short hash (to avoid long callback_data)
        file_hash = hashlib.md5(file_content.encode()).hexdigest()[:12]  # 12 char hash
        ...
        with FILE_STORAGE_LOCK:
            ...
            FILE_STORAGE = {k: v for k, v in FILE_STORAGE.items() if current_time - v['timestamp'] < 3600}
        
        # Ask user which check method to use (using short hash instead of file_id)
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(
            types.InlineKeyboardButton("✅ CHKR.CC Check", callback_data=f"bulk_chkr_{file_hash}"),
            types.InlineKeyboardButton("⚡️ Stripe SK Check", callback_data=f"bulk_stripe_{file_hash}")
        )
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('bulk_'))
def handle_file_check_callback(call):
    try:
        bot.answer_callback_query(call.id, "Processing...")
        
        # Extract hash from callback data
        # Format: bulk_chkr_HASH or bulk_stripe_HASH
        parts = call.data.split('_', 2)
        if len(parts) < 3:
            bot.edit_message_text("❌ Invalid request!", call.message.chat.id, call.message.message_id)
            return
        
        check_type = parts[1]  # 'chkr' or 'stripe'
        file_hash = parts[2]
        
        # Retrieve file content from storage
        with FILE_STORAGE_LOCK:
            if file_hash not in FILE_STORAGE:
                bot.edit_message_text("❌ File data expired or not found! Please upload the file again.", call.message.chat.id, call.message.message_id)
                return
            
            stored_data = FILE_STORAGE[file_hash]
            cc_list = stored_data['cards']
            
            # Check if expired (older than 1 hour)
            if time.time() - stored_data['timestamp'] > 3600:
                del FILE_STORAGE[file_hash]
                bot.edit_message_text("❌ File data expired! Please upload the file again.", call.message.chat.id, call.message.message_id)
                return
        ...
        if check_type == 'chkr':
            ...
            if len(cc_list) <= 200:
                ...
                for i, result in enumerate(results, 1):
                    ...
                    status = "💎" if "HIGH LIMIT" in result else "✅" if "LIVE" in result else "❌"
                    output += f"{i:2d}. {status} `{card_short}`\n"
                send_long_message(bot, call.message.chat.id, output, parse_mode='Markdown')
            ...
            if lives:
                ...
                if len(valid_cards) > 10:
                    lives_text += f"\n... and {len(valid_cards) - 10} more"
                send_long_message(bot, call.message.chat.id, lives_text, parse_mode='Markdown')
            
            if valid_cards:
                copyable_text = "📋 **COPY VALID CARDS:**\n\n```\n"
                copyable_text += "\n".join(valid_cards)
                copyable_text += "\n```"
                send_long_message(bot, call.message.chat.id, copyable_text, parse_mode='Markdown')
...
# Health Check HTTP Server (for UptimeRobot/keeping service awake)
        ...
        if self.path == '/health' or self.path == '/':
            ...
            response = {
                ...
                'stats': {
                    ...
                    'proxies': len(PROXY_POOL),
                    'sk_keys': len(SK_KEYS),
                    'bt_credentials': len(BT_CREDENTIALS)
                }
            }
